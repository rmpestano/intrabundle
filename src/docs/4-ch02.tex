...
Section \textit{2.2 - Java and OSGi} will introduce OSGi a framework for build service oriented Java modular applications. Finally section \ref{sec:forge} will introduce JBoss Forge, a Java framework used as runtime for Intrabundle.   
...
\section{JBoss Forge}
\label{sec:forge}
\href{http://forge.jboss.org/1.x/}{JBoss Forge} is a plugin based general purpose command line development tool. 
A Forge plugin can be as simple as a tool to print files to the console, or as complex as deploying an application to a server, tweeting\footnote{term used to refer as publishing messages to the twitter social media.} the status of your latest source-code commit, or even sending commands to a home-automation system or any computing task, the sky is the limit!.
