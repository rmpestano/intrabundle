...
\begin{itemize}
\item Standard Maven projects\footnote{Each project is a bundle}
\item Maven projects using BND tools\footnote{Each project is a bundle but bundle metadata is in pom.xml}  
\item Standard BND Tools project\footnote{Each project is a bundle but metadata is in bnd file};
\item Standard Eclipse Java projects\footnote{Each project is a bundle};
\item Package based bundles\footnote{In this kind of OSGi projects each package is a bundle};
\end{itemize}
...
Here is the justification of each attribute.\textbf{LoC} is being extracted because it is an indicative of high or low cohesion, if the component has too much lines of code its an evidence that it is probably doing more work then it should. \textbf{IPojo}, \textbf{Blueprint} and \textbf{Declarative Services} are recommended for managing OSGi services because they hide the "dirty work" of publishing and consuming services which sometimes may lead to incorrect behavior. \textbf{Stale Services References} refers to code that may retain OSGi service references from being collected by Java garbage collection even when the providing bundles are gone \citep{Gama 2012}. \textbf{Bundle dependencies} and \textbf{Required bundles} are closed related to coupling\footnote{which is a measure of how closely connected two software components are} between bundles. The less bundles a bundle depend the better it will be to maintain it as changes to other components will not affect it. \textbf{Declares permission} is a security software attribute in OSGi projects and may restrict access to bundles. \textbf{Publishes Interface} verifies if a bundle is exposing only its API and hide the implementation details from consumers. It is a good practice having functionalities that are independent of their respective implementations, this allows definitions and implementations to vary without compromising each other. Number of classes, interfaces and abstract classes are being collected to support the calculation of described attributes above. 
