...
\textbf{Stale Services References}\footnote{Refers to code that may retain OSGi service references even when the providing bundles are gone \citep{Gama 2012}} are detected via approximation, in other words, Intrabundle counts the number of gets and ungets\footnote{Operations that consume and release a service reference respectively} a bundle perform.
...
\textbf{Publishes interfaces} looks at bundle exported packages, if all exported packages contains only interfaces we say that bundle only imports interfaces. %It is a good practice expose only the API and hide the implementation.
\textbf{Declares permission} verifies if bundle implements security by contract via permission.perm file.
...
All the logic is inside \textbf{bundle} object, which is an immutable object\footnote{Is an object whose state cannot be modified after it is created. A good practice and core principle in domain driven design \citep{Evans 2003}}, in method \emph{getExportedPackages}. The bundle variable is provided by a Forge locator when user navigates to a directory which is an OSGi bundle, as explained in section \ref{sec:forge:locator}. 
...
\centering
\includegraphics[scale=0.8]{intrabundle-report}
\end{figure}  
\FloatBarrier
...
\centering
\includegraphics[scale=0.8]{intrabundle-report2}
\end{figure}  
\FloatBarrier
...
\centering
\includegraphics[scale=0.8]{intrabundle-metric-report}
\end{figure}  
\FloatBarrier
